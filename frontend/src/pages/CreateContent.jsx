        ...
        return (
          <>
            ...
            {pdfFile ? (
              ...
              <div className="mt-2 h-20 bg-gray-100 rounded flex items-center justify-center text-xs text-gray-400">No PDF uploaded</div>
            )}
            <div className="text-sm mt-3">
              {description ? <div dangerouslySetInnerHTML={{ __html: description.replace(/\n/g, "<br />") }} /> : <p className="text-gray-400 text-xs">No description yet</p>}
            </div>
      ...
      case "Multiple Image Text":
        return (
          <>
            <h3 className="font-bold text-sm">{title || "Content Preview"}</h3>
            {slides.length === 0 ? (
              <div className="text-gray-400 text-xs mt-4 text-center">No slides to preview</div>
            ) : (
                ...
                {slides.map((slide, idx) => (
                  <div key={idx} className="p-2">
                    ...
                    {slide.type === 'text' && slide.text && (
                      <div className="text-xs">{slide.text}</div>
                    )}
                    {slide.type === 'image' && !slide.imagePreview && (
                      <div className="text-xs text-gray-400">Empty image slide {idx + 1}</div>
                    )}
                    {slide.type === 'text' && !slide.text && (
                      <div className="text-xs text-gray-400">Empty text slide {idx + 1}</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-center">
              <div className="w-[220px] h-[460px] bg-[#20242c] rounded-[30px] p-3">
                <div className="bg-white rounded-[20px] h-full overflow-y-auto p-3">
                  ...
                  <p className="italic text-center mt-4 text-gray-500 text-xs">Swipe on!</p>
                </div>
              </div>
            </div>
          </div>
